...
//-----------------------------------------------------------------------------------
__host__ __device__ bool InList(int arg, int* list, int listSize)
{
...
// ----------------------------------------------------------------------------------
__host__ __device__ float dot(const float v1[_D_], const float v2[_D_]){
	float result = 0.0f;
	for_D_ result += (v1[d] * v2[d]);
	return result;
}
// --------------------------------------------------------------------------------
__host__ __device__ float mag(const float v[_D_]){
	return sqrt(dot(v,v));
}
//-----------------------------------------------------------------------------------
__host__ __device__ void diff_dir(const float this_v[_D_], const float minus_v[_D_], float _unit_dir[_D_]){
	float _dif[_D_];
...
//-----------------------------------------------------------------------------------
__host__ __device__ float CalculateRR(const float _rid[_D_], const float _rnab[_D_], float _dr[_D_]){
	float _r_[_D_];
...
//-----------------------------------------------------------------------------------
__host__ __device__ float CalculateLJ(const float _rr){
	return dev_Params._LJ_AMP*((dev_Params._2SIGMA6 / (_rr*_rr*_rr*_rr*_rr*_rr*_rr)) - (1.00000f / (_rr*_rr*_rr*_rr)));
}
//-----------------------------------------------------------------------------------
__host__ __device__ float CalculateLJ(const float _rr, const float sig, const float eps){
...
//---------------------------------------------------------------------------------
__host__ __device__ void Rotate2D(float _v[2], const float theta){
